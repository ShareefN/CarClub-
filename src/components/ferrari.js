...
class Ferrari extends Component{
  constructor(props){
	...
	componentWillMount(){
		fetch('/getFerrari').then(data => data.json())
		.then(data => {
			this.setState({
				cars: data,
			})
	...
	render(){
		return(
			...
			<h3 style={{fontSize: '35px', textAlign: 'center'}}>Ferrari</h3>
        ...
        <DialogActions>
          <Button href="/ferrari" color="primary" onClick={this.snedTD.bind(this)}>Request Test Drive</Button>
