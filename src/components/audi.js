...
class Audi extends Component{
  constructor(props){
	...
	componentWillMount(){
		fetch('/getAudi').then(data => data.json())
		.then(data => {
			this.setState({
				cars: data,
			})
	...
	render(){
		return(
			...
			<h3 style={{fontSize: '35px', textAlign: 'center'}}>Audi</h3>
        ...
        <DialogActions>
          <Button href="/audi" color="primary" onClick={this.snedTD.bind(this)}>Request Test Drive</Button>
