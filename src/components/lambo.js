...
class Lamborghini extends Component{
  constructor(props){
	...
	componentWillMount(){
		fetch('/getLambo').then(data => data.json())
		.then(data => {
			this.setState({
				cars: data,
			})
	...
	render(){
		return(
			...
			<h3 style={{fontSize: '35px', textAlign: 'center'}}>Lamborghini</h3>
        ...
        <DialogActions>
          <Button href="/LAMBO" color="primary" onClick={this.snedTD.bind(this)}>Request Test Drive</Button>
